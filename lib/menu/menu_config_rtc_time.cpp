...
bool menu_config_rtc_time(Button button, eMenu::Context& ctx)
{
    ...
    switch(current_step)
    {
        ...
        {
            if (button == Button::PRESS_UP)     { ctx.rtc.change_parameter(Parameter::HOUR, TypeOp::PLUS); }
            if (button == Button::PRESS_DOWN)   { ctx.rtc.change_parameter(Parameter::HOUR, TypeOp::MINUS); }
            if (button == Button::PRESS_OK)     { current_step = STEP_EDIT_MINUTES; }
        } break;

        case STEP_EDIT_MINUTES: 
        {
            if (button == Button::PRESS_UP)     { ctx.rtc.change_parameter(Parameter::MINUTE, TypeOp::PLUS); }
            if (button == Button::PRESS_DOWN)   { ctx.rtc.change_parameter(Parameter::MINUTE, TypeOp::MINUS); }
    ...
    if      (current_step == STEP_EDIT_HOURS)   { ctx.displ.show_clock(edit_clocks, BLINK_HOUR); } 
    else if (current_step == STEP_EDIT_MINUTES) { ctx.displ.show_clock(edit_clocks, BLINK_MIN);  }
